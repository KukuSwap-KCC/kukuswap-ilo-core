...
describe("KukuSwapPresale User Operation - Success Presale", function () {
    ...
    it("should be open for withdraw tokens after end", async function () {
        ...
        const ownerTokenBalanceBefore = await this.bnb.balanceOf(this.presaleOwner.address)

        await presale.connect(this.presaleOwner).addLiquidity()

        const ownerTokenBalanceAfter = await this.bnb.balanceOf(this.presaleOwner.address)
        //presale Owner
        ...
        expect(await this.wkcs.balanceOf(this.staking.address)).to.be.gt(ethers.BigNumber.from('0'));
...
describe("KukuSwapPresale User Operation - Force Fail", function () {
    const userAddress = ethers.utils.getAddress(
        "0x9280E3Eb147027FC04d4805f21D629eBCf305493"
    );
    ...
    it("should force fail from dev address", async function () {
        expect(
            await this.factory.presalesLengthByUser(this.presaleOwner.address)
        ).to.be.equal(1);
        ...
        const balanceOwnerAfter = await this.bnb.balanceOf(this.presaleOwner.address)

        expect(balanceOwnerAfter.sub(balanceOwnerBefore)).to.be.gt(ethers.BigNumber.from('0'))
