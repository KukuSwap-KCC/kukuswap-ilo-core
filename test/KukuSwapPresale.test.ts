...
describe("KukuSwapPresale User Operation - Success Presale", function () {
    ...
    before(async function () {
        ...
        await this.bnb.connect(this.bnbHolder).transfer(this.presaleOwner.address, ethers.utils.parseEther("500"))
    ...
    it("should create presale contract", async function() {
        ...
        await this.bnb.connect(this.presaleOwner).approve(this.generator.address, ethers.utils.parseEther("1000"))

        const startBlock = (await ethers.provider.getBlockNumber())+10;
        const endBlock = (await ethers.provider.getBlockNumber())+ 100;

        await this.generator.connect(this.presaleOwner).createPresale(
        ...
        expect(await this.factory.presalesLengthByUser(this.presaleOwner.address)).to.be.equal(1);

        const presaleIndex = (await this.factory.presalesLengthByUser(this.presaleOwner.address)) - 1;

        const newPresale = await this.factory.presaleAtIndexByUser(this.presaleOwner.address, presaleIndex);
        ...
        expect(presaleInfo.MAX_SPEND_PER_BUYER).to.be.equal(ethers.utils.parseEther("0.01"))
    ...
    it("should be open for deposit after start", async function() {
        ...
        await expect(presale.connect(this.investor).userDeposit(0, { value: ethers.utils.parseEther("0.01")})).to.be.revertedWith("NOT ACTIVE")
        ...
        await presale.connect(this.investor).userDeposit(0, { value: ethers.utils.parseEther("0.01")})

        let status = await presale.STATUS()

        expect(status.TOTAL_BASE_COLLECTED).to.be.equal(ethers.utils.parseEther("0.01"))
        expect(status. TOTAL_TOKENS_SOLD).to.be.equal(ethers.utils.parseEther("1"))
        expect(await this.investor.getBalance()).to.be.equal(userBalance1.sub(ethers.utils.parseEther("0.01")))
        

        //investor 2
        const userBalance2 = await this.investor2.getBalance()
        await presale.connect(this.investor2).userDeposit(0, { value: ethers.utils.parseEther("1")})

        status = await presale.STATUS()

        expect(status.TOTAL_BASE_COLLECTED).to.be.equal(ethers.utils.parseEther("0.02"))
        expect(status. TOTAL_TOKENS_SOLD).to.be.equal(ethers.utils.parseEther("2"))
        expect(await this.investor2.getBalance()).to.be.equal(userBalance2.sub(ethers.utils.parseEther("0.01")))
    })
    ...
    it("should be open for withdaraw tokens after end", async function() {
        ...
        await expect(presale.connect(this.investor).userWithdrawTokens()).to.be.revertedWith("AWAITING LP GENERATION")
        ...
        expect(await this.bnb.balanceOf(this.investor.address)).to.be.equal(ethers.utils.parseEther("1"))

        //investor 2
        await presale.connect(this.investor2).userWithdrawTokens()
        expect(await this.bnb.balanceOf(this.investor2.address)).to.be.equal(ethers.utils.parseEther("1"))

        expect(await this.locker.getUserNumLockedTokens(this.presaleOwner.address)).to.be.equal("1")
...
describe("KukuSwapPresale Manage Operation", function () {
    ...
    it("should manage only from presale owner", async function() {
        ...
        await expect(presale.connect(this.investor).setWhitelistFlag(true)).to.be.revertedWith("NOT PRESALE OWNER")

        await expect(presale.connect(this.investor).editWhitelist([this.investor2.address], true)).to.be.revertedWith("NOT PRESALE OWNER")

        await expect(presale.connect(this.investor).updateMaxSpendLimit(ethers.utils.parseEther("10"))).to.be.revertedWith("NOT PRESALE OWNER")

        await expect(presale.connect(this.investor).updateBlocks(startBlock, endBlock)).to.be.revertedWith("NOT PRESALE OWNER")

        await presale.connect(this.presaleOwner).setWhitelistFlag(true)

        await presale.connect(this.presaleOwner).editWhitelist([this.investor2.address], true)
        ...
        await expect(presale.connect(this.investor).userDeposit(0, { value: ethers.utils.parseEther("1")})).to.be.revertedWith("NOT WHITELISTED")

        await presale.connect(this.presaleOwner).updateMaxSpendLimit(ethers.utils.parseEther("10"))

        await presale.connect(this.investor2).userDeposit(0, { value: ethers.utils.parseEther("6")})

        const status = await presale.STATUS();

        expect(status.TOTAL_BASE_COLLECTED).to.be.equal(ethers.utils.parseEther("6.01"))

        await expect(presale.connect(this.presaleOwner).updateBlocks(startBlock, endBlock)).to.be.revertedWith("PRESALE IS ACTIVE")
    ...
    it("should allow the withdrawal of tokens  if presale is fail", async function() {
        ...
        expect(await this.investor2.getBalance()).to.be.equal(userBalanceBefore.add(ethers.utils.parseEther("6.01")))
        ...
        expect(await this.bnb.balanceOf(this.presaleOwner.address)).to.be.gte(ownerBalanceBefore)
...
describe("KukuSwapPresale Simple Operation - Round 1 with Early Access Tokens", function () {
    ...
    before(async function () {
        ...
        await this.settings.editEarlyAccessTokens(this.kuku.address, ethers.utils.parseEther("1"), true)
    ...
    it("should be open for deposit after start with whitelist round 1", async function() {
        ...
        //investor 1
        await expect(presale.connect(this.investor).userDeposit(0, { value: ethers.utils.parseEther("0.01")})).revertedWith("INSUFFICENT ROUND 1 TOKEN BALANCE")


        //investor 2
        const userBalance2 = await this.kukuHolder.getBalance()
        await presale.connect(this.kukuHolder).userDeposit(0, { value: ethers.utils.parseEther("0.1")})

        const status = await presale.STATUS()

        expect(status.TOTAL_BASE_COLLECTED).to.be.equal(ethers.utils.parseEther("0.01"))
        expect(status. TOTAL_TOKENS_SOLD).to.be.equal(ethers.utils.parseEther("1"))
        expect(await this.kukuHolder.getBalance()).to.be.equal(userBalance2.sub(ethers.utils.parseEther("0.01")))
        ...
        presale.connect(this.investor).userDeposit(0, { value: ethers.utils.parseEther("0.01")})
    })
...
describe("KukuSwapPresale Simple Operation with KUKU as Base Token", function () {
    ...
    it("should create presale contract", async function() {
        ...
        expect(presaleInfo.MAX_SPEND_PER_BUYER).to.be.equal(ethers.utils.parseEther("10"))
        expect(presaleInfo.AMOUNT).to.be.equal(ethers.utils.parseEther("100"))
    ...
    it("should be open for deposit after start", async function() {
        ...
        await this.kuku.connect(this.kukuHolder).approve(presale.address, ethers.utils.parseEther("100"))

        await presale.connect(this.kukuHolder).userDeposit(ethers.utils.parseEther("100"))
        ...
        expect(await this.kuku.balanceOf(this.kukuHolder.address)).to.be.equal(kukuBalanceBefore.sub(ethers.utils.parseEther("10")))
    ...
    it("should be open for withdaraw tokens after end", async function() {
        ...
        await expect(presale.connect(this.kukuHolder).userWithdrawTokens()).to.be.revertedWith("AWAITING LP GENERATION")
        ...
        //investor 1
        await expect(presale.connect(this.investor).userWithdrawTokens()).to.be.revertedWith("NOTHING TO WITHDRAW")
      
        //kukuholder
        await presale.connect(this.kukuHolder).userWithdrawTokens()
        expect(await this.bnb.balanceOf(this.kukuHolder.address)).to.be.equal(kukuBalanceBefore.add(ethers.utils.parseEther("10")))
