...
describe("KukuSwapPresaleSettings", function () {
    ...
    it("should have correct default settings", async function () {
        ...
        expect(await this.settings.getStakingAddress()).to.be.equal(this.staking.address)
    ...
    it("should not be manage from no owner address", async function() {
        ...
        await expect(this.settings.connect(this.user).setFeeAddress(this.staking.address)).to.be.revertedWith("Ownable: caller is not the owner")
        await expect(this.settings.connect(this.user).setFee(50)).to.be.revertedWith("Ownable: caller is not the owner")
        await expect(this.settings.connect(this.user).setRound1Length(1200)).to.be.revertedWith("Ownable: caller is not the owner")
        await expect(this.settings.connect(this.user).setMaxPresaleLength(93046)).to.be.revertedWith("Ownable: caller is not the owner")
    })
    ...
    it("should not be edited early access tokens from no owner address", async function() {
        await expect(this.settings.connect(this.user).editEarlyAccessTokens(this.kuku.address, 100, true)).to.be.revertedWith("Ownable: caller is not the owner")
    });

    it("should  be edited early access tokens from owner address", async function() {
        this.settings.connect(this.owner).editEarlyAccessTokens(this.kuku.address, 100, true)

        expect(await this.settings.earlyAccessTokensLength()).to.be.equal(1)

        let [token, holdAmount]: Address[] = await this.settings.getEarlyAccessTokenAtIndex(0)
    ...
    it("should corrent return if user holds sufficient Round1 token", async function() {
        ...
        expect(await this.settings.userHoldsSufficientRound1Token(this.owner.address)).to.be.equal(true)
        expect(await this.settings.userHoldsSufficientRound1Token(this.user.address)).to.be.equal(false)
    });
})
